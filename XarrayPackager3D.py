...
ds = xb.open_boutdataset(f"/home/jfkiewel/BOUT-next/examples/hasegawa-wakatani-3d/data/BOUT.dmp.*.nc")
...
xar.to_netcdf(path=f"Saved_NC_Files/{alpha}_{nout}.nc")

#Workaround to lazy loading preventing us from updating the XArray that we want to update.
xar["n"]=(["t","x","y","z"],ds["n"].values[:,:,:,:])
xar.to_netcdf(f"Saved_NC_Files/{alpha}_{nout}_temp.nc")
os.rename(f"Saved_NC_Files/{alpha}_{nout}_temp.nc",f"Saved_NC_Files/{alpha}_{nout}.nc")
